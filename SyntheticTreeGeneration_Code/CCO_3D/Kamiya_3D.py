# -*- coding: utf-8 -*-
"""
Created on Mon May 23 12:50:07 2016
...
#ci contains coordinate xi,yi,zi of the point
#f contains f0,f1,f2
...
def calculate_segment_lengths(c0,c1,c2,x,y,z):
    if (len(c0) != 3) or (len(c1) != 3) or (len(c2) != 3):
        print "error in calculate_segment_lengths inputs"
        return 0.,0.,0.
    l0 = np.sqrt( (x-c0[0])**2 + (y - c0[1])**2 + (z-c0[2])**2)
    l1 = np.sqrt( (x-c1[0])**2 + (y - c1[1])**2 + (z-c1[2])**2)
    l2 = np.sqrt( (x-c2[0])**2 + (y - c2[1])**2 + (z-c2[2])**2)
    return l0,l1,l2
...
def kamiya_loop_r2(x_ini, y_ini, z_ini, c0, c1, c2, f, r):
    l = np.zeros(3)
    l[0],l[1],l[2] = calculate_segment_lengths(c0,c1,c2,x_ini,y_ini,z_ini)
    ...
    if (r[0] == 0.):
        return False, x_ini, y_ini, z_ini, np.sqrt(r), l
    else:        
...
def kamiya_single_bif_opt(r, f, c0, c1, c2, iter_max, tolerance, store_whole_results):
    ...
    while it < iter_max and gdt > tolerance:     
        cvge, x_c, y_c, z_c, r_c, l = kamiya_loop_r2(x, y, z, c0, c1, c2, f, r)
        if (cvge):
            vol_c = single_bif_volume(r_c,l) 
            gdt = np.sqrt((x_c - x)**2 + (y_c - y)**2 + (z_c - z)**2)       
            if (gdt < tolerance) and (vol_c < volume):              
                ...
                print  "optimal position ", x_c, y_c, z_c
                print "optimal radii ", np.sqrt(r)
                storage.append([x_c, y_c, z_c, r])                
                return True, storage #x_c, y_c, r
            else:
                it = it + 1
                x , y , z, volume, r = x_c, y_c, z_c, vol_c, r_c
                if store_whole_results:
                    storage.append([x_c, y_c, z_c, r]) #### need it if used in Kamyia testing 
