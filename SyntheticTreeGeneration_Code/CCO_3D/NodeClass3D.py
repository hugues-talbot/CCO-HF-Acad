...
class Tree:
    ...
    def test_connection(self, old_child_index, new_child_location):
        ...
        #check boundaries
        if (convergence == True):                     
            branching_location = np.array([res[0][0], res[0][1], res[0][2]]) 
            radii = res[0][3] 
            print "radii", radii
        ...
        if (convergence == True):
            #make connection                       
            betas = cco_3df.calculate_betas(radii[1]/radii[2], 3.) #this involves old_child will be child_0, new_child is child_1
