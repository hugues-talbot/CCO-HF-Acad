...
\maketitle
	...
	\end{equation}
		...
		\begin{figure}
			...
			\caption{The notion of osculating (best local approximation) defines the curvature as the inverse of the radius of the circle.\label{fig:oscul}}
		\end{figure}
	
	More precisely, for any curve, we can locally approximate a sufficiently regular curve (${\cal C}^2$ is enough) at a point by a tangent circle that best approximates it locally (see Fig.~\ref{fig:oscul}). This circle is called an {\em osculating circle}. The radius of its radius defines the curvature.
	

	
	Another way to define the curvature is to consider a point along the curve $C$. The variation of the tangent vector along the curve defines the curvature
	...
	are equivalent (see Fig.~\ref{fig:theta}). 
